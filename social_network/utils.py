...
def build_model(args, dataset, device: torch.device, model_init_seed=None):
    ...
    if args.model == 'MM_GNN':
        ...
        model = MMGNN(dataset, args.num_layer, args.moment, use_norm=args.use_norm, hidden=args.hidden, mode=args.mode, use_adj_norm=args.use_adj_norm, use_center_moment=args.use_center_moment, device=device)
    else:
